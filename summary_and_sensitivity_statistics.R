...
# summary_all <- epidemiological %>% group_by(environmental_variables_cleaned, coefficient_type, continuous_or_dichotomised) %>% summarize(mean_all = mean(coefficient), min_all = min(coefficient), max_all = max(coefficient))

# Ignore function: an attempt at collating everything in one function
# summary_presented <- epidemiological %>% filter(`CASP TOTAL` != "POOR" & statistical_significance != "insignificant") %>% group_by(environmental_variables_cleaned, coefficient_type, continuous_or_dichotomised) %>% summarize(presented = paste(round(mean(coefficient[coefficient_type != "regression slope"]), 2), round(min(coefficient[coefficient_type != "regression slope"]), 2), round(max(coefficient[coefficient_type != "regression slope"]), 2), sep = ", "),                                                                                                                                                                                                                              presented2 = paste(round(mean(coefficient[coefficient_type == "regression slope"]), 4), round(min(coefficient[coefficient_type == "regression slope"]), 4), round(max(coefficient[coefficient_type == "regression slope"]), 4), sep = ", "))
...
# summary <- full_join(summary, summary_all, by = c("environmental_variables_cleaned", "coefficient_type", "continuous_or_dichotomised"))
...
# summary <- data.frame(lapply(summary, function(x) if(is.numeric(x)) round(x, 2) else x))
